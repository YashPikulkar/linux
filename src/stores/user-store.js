...
export const useUserStore = defineStore('user', {
  ...
  actions: {
    ...
    async fetchUserApplications() {
      if (!this.uid) {
        this.applicationsError = 'No user logged in'
        return
      }
      ...
      try {
        const response = await fetch(`${baseUrl}/application/user/${this.uid}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${this.token}`, // if your API uses JWT
          },
        })
        ...
        if (!response.ok || !resJson.success) {
          ...
          this.applicationsError = resJson.message || 'Failed to fetch applications'
          return
        }

        this.appliedApplications = resJson.data || []
      } catch (error) {
        console.error('Error fetching user applications:', error)
        this.applicationsError = 'Network error or server unavailable'
      } finally {
    ...
    async updateRecruiter(editData) {
      // 🔹 Log what the store received
      console.log(
        '[UserStore] updateRecruiter received editData:',
        JSON.parse(JSON.stringify(editData)),
      )

      try {
        // Normalize email: send null if empty
        if (!editData.companyEmail) {
          editData.companyEmail = null
          console.log('[UserStore] companyEmail normalized to null')
        }
        ...
        if (response.ok) {
          ...
          console.log('[UserStore] updateRecruiter response JSON:', resJson)

          // Update local store
          this.company = JSON.parse(JSON.stringify(editData))
          console.log('[UserStore] Local company updated:', this.company)

          return { message: 'success' }
        } else {
          const errorData = await response.json().catch(() => null)
          console.warn('[UserStore] Backend returned error:', errorData)

          return { message: 'failed', error: errorData }
        }
      } catch (err) {
        console.error('[UserStore] Update recruiter error:', err)
