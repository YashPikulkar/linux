...
export const useCompanyStore = defineStore('company', { // Fixed: changed from 'job' to 'company'
  state: () => ({
    company: null,
    loading: false,
    error: null,
    ...
    async fetchCompanyByRecruiter() {
      ...
      try {
        ...
        if (!contentType || !contentType.includes('application/json')) {
          ...
          throw new Error('Server returned non-JSON response. Please check if the backend server is running and the endpoint exists.');
        }
        ...
        if (this.company && this.company.locations) {
          if (typeof this.company.locations === 'string') {
            this.company.locations = this.company.locations.split(',').map(loc => loc.trim()).filter(loc => loc);
