...
• Knowledge of modern build tools (Vite, Webpack)`,
        skills: ['Vue.js', 'JavaScript', 'TypeScript', 'HTML', 'CSS', 'REST APIs'],
        postedAt: '2024-01-15',
        status: 'published',
        applicants: [
          {
    ...
    jobCategories: [
      ...
      'Hospitality', 'Real Estate', 'Telecommunications', 'Marketing',
      'Sales', 'Design', 'Other'
    ],
    popularSkills: [
      ...
      'Git', 'REST APIs', 'GraphQL', 'Redux', 'Vuex',
      'Jest', 'Cypress', 'Selenium', 'Jenkins', 'CI/CD'
    ],
    filters: {
      jobType: '',
      modeOfWork: '',
  ...
  getters: {
    ...
    getFilteredJobs: (state) => {
      ...
      const { jobType, modeOfWork, experienceLevel, location, skills, salaryRange } = state.filters
      ...
      if (modeOfWork) {
        filteredJobs = filteredJobs.filter(job => job.modeOfWork === modeOfWork)
      }

      if (experienceLevel) {
        filteredJobs = filteredJobs.filter(job => job.experienceRequired === experienceLevel)
      }
    ...
    getJobStats: (state) => (companyName) => {
      const companyJobs = state.jobs.filter(job => job.company === companyName)
      return {
        totalJobs: companyJobs.length,
        totalApplicants: companyJobs.reduce((total, job) => total + (job.applicants?.length || 0), 0),
        pendingApplicants: companyJobs.reduce((total, job) =>
          total + (job.applicants?.filter(app => app.status === 'pending').length || 0), 0),
        acceptedApplicants: companyJobs.reduce((total, job) =>
          total + (job.applicants?.filter(app => app.status === 'accepted').length || 0), 0),
        rejectedApplicants: companyJobs.reduce((total, job) =>
          total + (job.applicants?.filter(app => app.status === 'rejected').length || 0), 0)
      }
    }
  },
  ...
  actions: {
    addJob(jobData) {
      const newJob = {
        ...jobData,
        id: this.jobs.length > 0 ? Math.max(...this.jobs.map(j => +j.id || 0)) + 1 : 1,
        applicants: [],
    ...
    addApplicant(jobId, applicantData) {
      ...
      if (job) {
        const newApplicant = {
          ...applicantData,
          id: job.applicants.length > 0 ? Math.max(...job.applicants.map(a => a.id)) + 1 : 1,
          status: 'pending',
    ...
    getJobAnalytics(jobId) {
      ...
      return {
        ...
        acceptanceRate: total ? ((counts.accepted || 0) / total * 100).toFixed(1) : 0,
        rejectionRate: total ? ((counts.rejected || 0) / total * 100).toFixed(1) : 0
      }
    }
  }
})
