<template>
  <BaseFilterCard
    title="Companies"
    :subtitle="`Selected: ${selectedCompanies.length || 'None'}`"
    :is-active="isActive"
  ...
  >
    ...
    >
      <template v-slot:append>
        <q-icon v-if="selectedCompanies.length === 1" name="business" class="q-ml-xs" />
        <q-icon v-else-if="selectedCompanies.length > 1" name="domain" class="q-ml-xs" />
      </template>
    </q-select>

    <!-- Selected Companies -->
    <div v-if="selectedCompanies.length" class="companies-subtitle q-mb-xs">Selected Companies</div>
    ...
    <!-- Popular Companies -->
    <div v-if="filteredCompanies.length" class="companies-subtitle">Popular Companies</div>
...
export default {
  ...
  methods: {
    ...
    handleNewCompany(inputValue) {
      ...
      const match = this.store.companies.find((c) => c.label.toLowerCase() === normalized)
