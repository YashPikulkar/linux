<template>
  <BaseFilterCard
    title="Skills"
    :subtitle="selectedSkills.length ? `Selected: ${selectedSkills.length}` : 'None'"
    :active="isActive"
    @clear="clearSkills"
  >
    <!-- 🔽 Dropdown Select -->
    ...
    >
      <template v-slot:append>
        <q-icon v-if="selectedSkills.length === 1" name="psychology" class="q-ml-xs" />
        <q-icon v-else-if="selectedSkills.length > 1" name="engineering" class="q-ml-xs" />
      </template>
    </q-select>

    <!-- 🔷 Selected Skills -->
    <div v-if="selectedSkills.length" class="skills-subtitle q-mb-xs">Selected Skills</div>
  ...
  computed: {
    ...
    filteredSkills() {
      ...
      return (this.store.skills || [])
        ...
        .filter((skill) => skill.label && skill.label.toLowerCase().includes(searchLower))
  ...
  methods: {
    ...
    onDropdownFilter(val, update) {
      ...
      this.filteredDropdownOptions = (this.store.skills || [])
        .filter((skill) => skill && !this.selectedSkills.includes(skill.value))
        .filter((skill) => skill.label && skill.label.toLowerCase().includes(search))
