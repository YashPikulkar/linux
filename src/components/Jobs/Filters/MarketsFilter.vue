<template>
  <BaseFilterCard
    title="Markets"
    :subtitle="selectedMarkets.length ? `Selected: ${selectedMarkets.length}` : 'None'"
    :active="isActive"
    @clear="clearMarkets"
  >
    <!-- 🔽 Dropdown Select -->
    ...
    >
      <template v-slot:append>
        <q-icon v-if="selectedMarkets.length === 1" name="work" class="q-ml-xs" />
        <q-icon v-else-if="selectedMarkets.length > 1" name="business" class="q-ml-xs" />
      </template>
    </q-select>

    <!-- 🔷 Selected Markets -->
    <div v-if="selectedMarkets.length" class="markets-subtitle q-mb-xs">Selected Markets</div>
...
export default {
  ...
  computed: {
    selectedMarkets: {
      ...
      set(val) {
        this.store.selectedMarkets = Array.isArray(val) ? val : val ? [val] : []
      },
    },
    filteredMarkets() {
  ...
  methods: {
    ...
    handleNewMarket(inputValue) {
      ...
      const match = this.store.markets.find((m) => m.label.toLowerCase() === normalized)
