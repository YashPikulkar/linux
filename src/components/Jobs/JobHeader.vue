  ...
  <div class="q-mb-md job-tabs-wrapper">
    ...
    <div class="row items-center q-gutter-md tabs-row">
      ...
      >
        ...
        <!-- Badge for Saved tab -->
        <q-badge
          v-if="tab.name === 'saved' && savedJobsCount > 0"
          color="grey-4"
          text-color="black"
          :label="savedJobsCount"
...
export default {
  ...
  computed: {
    ...
    // ✅ count saved jobs from jobs-store (Set.size)
    savedJobsCount() {
      return this.jobsStore.savedJobIds.size
    },
    // ✅ only show "saved" tab if user logged in
    visibleTabs() {
